...
class NetworkManager: NSObject, URLSessionDelegate {
    ...
    func fetchObjectPoi(uuid : String)
    {
        ...
        sessionLoadObjectPoi.dataTask(with: url) { [self] (data,response,error) in
            if let response = response {
                    ...
                    for objP in objPoiArray
                    {
                        ...
                        if ( audioUUID != nil )
                        {
                            for objAudioUUID in audioUUID!
                            {
                                ...
                                self.fetchAudioGuidePoiLocal(uuid : uuidItem!, uuidProvide: uuidProvide!, uuidAudio: getAudioObj!)
                            }
                        }
                        // fetch image [array]
                        ...
                        if ( imageUUID != nil)
                        {
                            for objImageUUID in imageUUID!
                            {
                                ...
                                self.fetchImagePoiLocal(uuid : uuidItem!,uuidProvide: uuidProvide!, uuidImage: getImageObj!)
                            }
                        }
                        // fetch title:
